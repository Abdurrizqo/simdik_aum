        ...
        <div class="modal-action flex justify-center">
            <form method="dialog" class="flex justify-center gap-8">
                ...
                <a id="linkGantiStatus"
                    class="btn-cus border-secondary border flex justify-center items-center rounded-md hover:text-white text-secondary w-40 hover:bg-secondary">Aktifkan</a>
            </form>
        </div>
    </div>
</dialog>
...
@section('content')
    <div class="w-full">
        <div class="mb-8">
            <h1 class="text-2xl font-bold">Daftar Pegawai Non Aktif</h1>
        </div>
                    ...
                    @foreach ($user as $index => $item)
                        <tr>
                            ...
                            <td class="flex gap-8 justify-center">
                                <a href="user/detail/{{ $item['idUser'] }}" class="btn btn-primary rounded w-20">Detail
                                    User</a>

                                <button class="btn-success btn text-white rounded w-20"
                                    onclick="my_modal_3.showModal(); 
                                    modalGantiStatus(
                                    'Apakah Anda Yakin Untuk Mengaktifkan Data Pegawai {{ $item['nickname'] }}', 
                                    '',
                                    'user/aktifkan/{{ $item['idUser'] }}')">
                                    Aktif
                                </button>

                                {{-- <a href="user/aktifkan/{{ $item['idUser'] }}"
                                    class="btn-success btn text-white rounded w-20">Aktif</a> --}}
                            </td>
                        </tr>
                    @endforeach
                </tbody>
